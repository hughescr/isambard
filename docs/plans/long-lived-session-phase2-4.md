...
## Q12 — Phase 2 leftovers: calendar diff, health pushed on change, perch decision

**Design phase:** 2 (amendment B6)

**Goal:** Conductor-mode Discord envelopes today carry NO calendar and NO service-health section at all (`conductor-processor.ts` assembles `buildDiscordEnvelope` from `contextPolicy` deltas only; the one-shot `buildUserMessagePrefix` path is what carried them). Add both back the way design section 3.1 specifies — the day's agenda only when it changed, service health only when it changed — through the same `ContextPolicy` mark/delta idiom Q9 used for the state top set, and pin the perch decision: perch turns keep the full context (`buildPerchContext`: full agenda, full health, top-3 state) and receive none of the Discord deltas.

**Depends on:** Q9 · **Deployable:** yes · **Points:** 8

**Fixed contracts between slices (binding; slice C imports these before A and B finish):**

- `src/agent/session/calendar-delta.ts` (new, pure): `export interface AgendaEntry { uid: string, recurrenceId?: string, start: string /* ISO */, end: string, summary: string, location?: string, status?: string, isAllDay: boolean }`; `export function agendaKey(entry: AgendaEntry): string` (= `uid` + `'|'` + `recurrenceId ?? ''`); `export function agendaFingerprint(entry: AgendaEntry): string` (stable JSON of start/end/summary/location/status/isAllDay); `export function diffAgenda(baseline: readonly AgendaEntry[] | undefined, current: readonly AgendaEntry[]): { added: AgendaEntry[], removed: AgendaEntry[], changed: AgendaEntry[], isFirst: boolean }` — `isFirst: true` and all lists empty when `baseline` is `undefined`; otherwise keyed by `agendaKey`, `changed` = same key, different fingerprint. `export function dayWindow(nowMs: number, timezone: string): { startMs: number, endMs: number }` — local-calendar-day bounds in `timezone` (luxon). `export function toAgenda(events: readonly CalendarEvent[], window: { startMs: number, endMs: number }): AgendaEntry[]` — keeps only events overlapping the window, sorted by start then key. Day-aligned windows are what make the CalDAV rolling query window churn-free: two polls on the same local day compare the same day.
- `ContextBuilder` (`src/agent/context-builder.ts`): `loadCalendarAgenda(userId: string, now: Date): Promise<CalendarEvent[]>` — the per-user fetch `#buildCalendarSection` already does (registry `getAllCalendars(userId)` then `client.getContextEvents(servers, now)`), returning `events` only; `[]` when no service, no servers, or on error (logged at warn). No formatting.
- `ContextPolicy` (`src/agent/session/context-policy.ts`) gains: `calendarDelta(userId: string, timezone: string): Promise<CalendarDelta>` where `CalendarDelta = { agenda: AgendaEntry[], events: CalendarEvent[] /* the raw window events, for formatCalendarContext */, added: AgendaEntry[], removed: AgendaEntry[], changed: AgendaEntry[], isFirst: boolean, polled: boolean }`; `markCalendarSeen(userId: string): void` (stores the agenda from the most recent `calendarDelta` call for that user as the baseline); `healthNote(): string | undefined` (the current `healthRegistry.buildStatusSummary()` rendered as the `[Service health]` body when it differs from the last marked summary — including the all-online transition, rendered as `All services are back online.` — else `undefined`); `markHealthSeen(): void`; `resetAll()` also clears calendar baselines, the poll cache, and the health mark. `CreateContextPolicyParams` gains `healthRegistry?: Pick<ServiceHealthRegistry, 'buildStatusSummary'>`, `calendarPollIntervalMs?: number` (default 60 minutes) and widens `contextBuilder` to `EventsDeltaSource & StateTopSetSource & CalendarAgendaSource` (`Pick<ContextBuilder, 'loadCalendarAgenda'>`). The "hourly host poll" is implemented as a per-user cache: `calendarDelta` re-fetches only when the cached agenda for that user is older than `calendarPollIntervalMs` or the local day (per `dayWindow`) has rolled over; `polled` reports whether a fetch happened. When nothing was polled the delta is empty (an unchanged cache is by definition unchanged).
- `BuildDiscordEnvelopeParams` (`src/agent/session/envelope.ts`) gains `calendarChanged?: { agenda: string, added: string[], removed: string[], changed: string[], isFirst: boolean }` rendered as a `[Calendar]` section: on `isFirst` the full `agenda` text; otherwise a `+/-/~` change list (one `HH:mm–HH:mm summary` line per entry) followed by the full `agenda` text; omitted when `undefined`. `healthNote` (already a param, currently never fed) keeps rendering as `[Service health]`.

### Files & changes

- **Slice A (calendar primitive):** `src/agent/session/calendar-delta.ts` (new, per contract) + `tests/unit/agent/session/calendar-delta.test.ts` (table-driven: first poll is `isFirst` with no lists; add/remove/change by key; a recurring instance with a different `recurrenceId` is a different key; identical agendas across a window that slides within the same local day produce an empty delta; `dayWindow` across a DST change and a UTC-negative zone; `toAgenda` drops events entirely outside the day and keeps ones straddling midnight); `src/agent/context-builder.ts` `loadCalendarAgenda` on the `ContextBuilder` interface and impl (refactor `#buildCalendarSection` to call it so the fetch lives once) + tests in `tests/unit/agent/context-builder-loading.test.ts`; barrel export in `src/agent/session/index.ts`.
- **Slice B (policy + envelope):** `src/agent/session/context-policy.ts` calendar cache/delta/mark, health note/mark, `resetAll` widening, per contract + `tests/unit/agent/session/context-policy.test.ts` (fake clock: no refetch inside the poll interval, refetch after it, refetch on day rollover; `markCalendarSeen` before any delta call is a no-op; per-user isolation; health note appears once per change and again on the return to all-online; `resetAll` re-arms both); `src/agent/session/envelope.ts` `calendarChanged` rendering + `tests/unit/agent/session/envelope.test.ts` exact-string cases; `src/agent/session/types.ts` only if a shared type is genuinely needed (A1: single owner).
- **Slice C (wiring + perch decision):** `src/integrations/discord/setup/conductor-processor.ts` — add `contextPolicy.calendarDelta(first.userId, timezone)` (timezone resolved from `storedTimezone` BEFORE the delta call, so reorder the `Promise.all` accordingly) and `contextPolicy.healthNote()` to the envelope, feeding `calendarChanged` (agenda text = `formatCalendarContext(delta.events, now, timezone)` imported from `@/integrations/caldav`, which both `src/agent/context-builder.ts` and `src/integrations/discord/bot.ts` already import from, so the boundaries rule allows it; the `+/-/~` lines come from the `AgendaEntry` lists); after a non-withdrawn submit call `contextPolicy.markCalendarSeen(first.userId)` and `contextPolicy.markHealthSeen()` next to the existing `markEventsSeen`/`markStateTopSetSeen`; tests in `tests/unit/integrations/discord/setup/conductor-processor.test.ts` (exact `buildDiscordEnvelope` args; marks only on non-withdrawn submit). `src/app/sessions.ts` — thread `healthRegistry` into `createContextPolicy` for the conversation conductor (add it to `CreateConversationConductorParams`, pass from `src/index.ts` where the registry exists; verify the name there at pickup) + `tests/unit/app/sessions.test.ts`. **Perch decision:** `src/agent/perch/envelope.ts` module doc and `src/app/sessions.ts` `createPerchConductor` comment state that perch turns deliberately carry the full `buildPerchContext` block (full agenda, health, top-3 state) and none of the Discord deltas — perch runs a handful of turns per day and its context must be self-contained; `tests/unit/agent/perch/envelope.test.ts` pins that `buildPerchSlotEnvelope` renders the supplied context verbatim with no delta sections. `docs/architecture.md` Session Mode: one paragraph on the Discord delta sections (events, state, calendar, health) vs the perch full block.

### Tests first

- `diffAgenda`: first poll → `isFirst`, empty lists; add/remove/change detection by `uid|recurrenceId`; same-day window slide → empty delta
- `dayWindow` in `America/Los_Angeles` on a DST-change day and in `Pacific/Auckland`
- policy: poll cadence on a fake clock; day rollover forces a poll; marks are per user; `resetAll` re-arms
- policy: health note on offline transition, silent while unchanged, note again on return to all-online, `undefined` with no registry
- envelope: `[Calendar]` first-time renders only the agenda; later renders change list + agenda; absent when undefined; `[Service health]` from `healthNote`
- conductor-processor: exact args; timezone resolved before `calendarDelta`; marks only after a non-withdrawn submit; a `calendarDelta` rejection is logged and the envelope still goes out without a calendar section
- perch: envelope has no delta sections; `createPerchConductor` builds no `ContextPolicy`

### Acceptance

- `bun test`, typecheck, lint, ast-grep clean; mutation 100% on `calendar-delta.ts` and on every touched hunk in `context-policy.ts`, `envelope.ts`, `context-builder.ts`, `conductor-processor.ts`, `sessions.ts`, `perch/envelope.ts`
- With no calendar service configured and an all-online registry, the Discord envelope is byte-identical to Q9's output (asserted in the conductor-processor test)
- boundaries clean: `calendar-delta.ts` depends on luxon and `@/integrations/caldav` types only via `src/agent`'s existing allowed imports (check how `context-builder.ts` imports `CalendarEvent` today and mirror it)
