...
- [ ] **"Perch context block in perch envelopes (partly landed in Q12)" has no completion check.** No package in this plan verifies what Q12 left outstanding or closes the remainder now that Q1/Q2 add pull tools for rejections and health. Needs either a small verification package or an explicit note folded into Q1/Q2 that the existing push sections are intentionally left as-is.
