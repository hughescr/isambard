...
## Weekly source cross-check (#137)

When vector indexing and the embedder are available, Izzy enrolls the local SQLite file for its first cross-check **seven days after its first open**; there is no DynamoDB scan at boot. The singleton `vector_cross_check_state` table stores the next due time, last completed page cursor and last run time across restarts. Overdue restarts wait a short grace period; failures persist a one-hour retry deadline. Shutdown aborts active reads and pacing waits and awaits the job before closing the indexer, SQLite or DynamoDB client. The local hourly TTL prune remains independent.

The job pages local rowids, reads full DynamoDB items in four-key, eventually consistent BatchGetItem requests, and compares the SHA-256 of `path + newline + content` and TTL. `updatedAt` alone is **not** a change detector: reading a memory bumps it without changing its content. A content mismatch is re-read strongly before it is queued for re-indexing; a TTL-only mismatch uses `setTtls`. Candidate orphans require a strongly consistent confirmation before a generation-guarded delete and tombstone. A page with half or more of its local keys absent is rejected without deletions or checkpoint advancement (possible wrong table/stage). Malformed items also fail closed. Each batch's confirmed orphan is deleted before another request or pacing wait. After indexing, the job drains the queue **and verifies the derived hashes** before checkpointing; a drain alone cannot guarantee jobs succeeded.
