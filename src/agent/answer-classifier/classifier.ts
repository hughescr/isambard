...
export interface AnswerClassifier {
   ...
   */
    classify(question: PendingQuestion, message: MessageToClassify): Promise<ClassificationResult>
}

/**
 * Create an answer classifier instance.
 */
export function createAnswerClassifier(config?: ClassifierConfig): AnswerClassifier {
    return {
        classify: async (question: PendingQuestion, message: MessageToClassify): Promise<ClassificationResult> => {
            ...
            if(config?.classifyWithLLM) {
                return await config.classifyWithLLM(question, message);
            }
