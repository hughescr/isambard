...
Claude's shared five-hour and weekly windows can each bind. Codex is a separate shared subscription: respect only the bucket scopes and reset times actually reported. DeepSeek is pay-as-you-go; its balance is money, and input, cached input, and output tokens debit a real but often tiny API cost. It can conserve scarce subscription headroom when it is capable. The per-turn \`Quota:\` JSON is keyed by provider: \`quota_lookup.status\` says whether the quota reading is known, \`observed_at\` dates the quota values, and \`resets_at\` dates a bucket reset. A quota API lookup failure does not establish whether inference is available or quota is exhausted.
