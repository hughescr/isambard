 ...
 * {@link DiscordCapability} — mirrors {@link sendOneChunkToWellKnownChannel}'s
 * `catching_up`/`perching` -> `catch_up_output`/`perch_output` mapping; every other envelope kind
 * queues as a plain `agent_response`.
 */
 ...
 * available: every chunk routes through its outbox fallback, mirroring {@link sendResponse}'s own
 * facade path. Split out purely to keep `sendEnvelopeResponse` under the project's complexity
 * threshold — no behavioural difference from being inlined.
 */
...
async function sendChunksViaClient(
    ...
    try {
        ...
        for(let i = 0; i < chunks.length; i++) {
            ...
            if(chunk === undefined) {
                // Stryker disable next-line StringLiteral,CallExpression: invariant violation — matches this file's other identical unreachable guards (sendChunksByReply/sendChunksToChannel/sendChunksToWellKnownChannel); debug context only
...
interface SendEnvelopeResponseConfig {
     ...
     * outbox fallback when Discord is offline — exactly as {@link sendResponse}'s own facade path
     * does. Every production call site wires this; omitting it is a test-only convenience, and a
     * send failure with no facade is reported honestly as `{ sent: false }` rather than a
     * fabricated `queued: true` (see the module-level history: P10 originally faked `queued: true`
     * on any failure here, which journaled a lost response as delivered).
     */
    discordCapability?: DiscordCapability
}
 ...
 * response. A missing well-known channel for `catchup`/`perch` has no channel to fall back to
 * (unlike {@link sendResponse}'s `useFallbackOnError`), so it resolves to `{ sent: false,
 * skipReason }` instead. The `@@NO_RESPONSE@@` sentinel resolves to `{ sent: false, skipReason:
 * 'no-response' }`. When `config.discordCapability` is provided (every production call site wires
 * one), a chunk queued or unavailable through it resolves to `{ sent: false, queued: true }`,
 * mirroring {@link sendResponse}'s own facade path, and the caller (P10's boot sequence, via
 * `Conductor.deliver`) treats that the same as never having sent at all, safe to retry on a later
 * boot — the outbox itself owns the retry. Without a `discordCapability`, there is no outbox to
 * fall back to, so a channel that can't be fetched or a chunk send that still fails after retries
 * resolves to a genuine `{ sent: false }` rather than a fabricated `queued: true`.
 *
