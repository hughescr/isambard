...
import { sendResponseToWellKnownChannel, sendEnvelopeResponse } from '../response-sender';
...
import {
    type ClaudeAgent, type PerchConfig, type ActivityLogger, type Conductor, type SessionJournal, type Envelope, type UndeliveredEnvelope,
    computeRecovery, buildDiscordEnvelope, buildCatchupEnvelope, runBootSequence
} from '@/agent';
import type { ServiceHealthRegistry } from '@/services';
import { formatTimeSince, getTimeOfDay, resolveTimezone, formatTimeHeader } from '@/utils';
...
interface SetupInboxParams {
    ...
    /**
     * P10 conductor-mode dependencies. When every one of these is present, `runInboxInit` calls
     * {@link runConductorInboxInit} instead of the legacy catch-up-runner branch below; the
     * oneshot branch (any of these omitted) is byte-for-byte unchanged.
     */
    conversationConductor?: Conductor
    journal?:               SessionJournal
    responseRouter?:        ResponseRouter
 ...
 * discord-online path, when Discord was not yet available at call time (P10).
 */
...
export function setupInboxAndCatchUp(params: SetupInboxParams): Promise<void> {
    ...
    /**
     * Every P10 conductor dependency, present only when the caller actually wired the conductor
     * branch — pulled into its own function (rather than one long `&&` chain inline in
     * `runInboxInit`) so that function's own cyclomatic complexity stays under the project's
     * lint threshold.
     */
    function resolveConductorDeps(): RunConductorInboxInitParams | undefined {
        if(!conversationConductor || !journal || !responseRouter || !rateLimiter || !ingressGate) {
            return undefined;
        }
        return {
            inboxManager, readyClient, botStateManager, perchConfig, ingressGate,
    ...
    async function runInboxInit(): Promise<void> {
        try {
            ...
            const conductorDeps = resolveConductorDeps();
            if(conductorDeps) {
                await runConductorInboxInit(conductorDeps);
                return;
            }
