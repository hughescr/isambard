...
describe('syncAgentsAndSkills', () => {
    ...
    test('should create target agents and skills directories if they do not exist', async () => {
        ...
        await mockFsPromises.mkdir(join(tempSourceRoot, 'agents'), { recursive: true });
        await mockFsPromises.mkdir(join(tempSourceRoot, 'skills'), { recursive: true });
        await mockFsPromises.writeFile(join(tempSourceRoot, 'agents', 'test-agent.md'), '# Test Agent');
        await mockFsPromises.writeFile(join(tempSourceRoot, 'skills', 'test-skill.md'), '# Test Skill');
        ...
        const agentsContents = await mockFsPromises.readdir(join(tempTargetRoot, 'agents'));
        const skillsContents = await mockFsPromises.readdir(join(tempTargetRoot, 'skills'));
    ...
    test('should copy agents from source to target', async () => {
        ...
        await mockFsPromises.mkdir(join(tempSourceRoot, 'agents'), { recursive: true });
        await mockFsPromises.mkdir(join(tempSourceRoot, 'skills'), { recursive: true });
        await mockFsPromises.writeFile(join(tempSourceRoot, 'agents', 'agent1.md'), '# Agent 1');
        await mockFsPromises.writeFile(join(tempSourceRoot, 'agents', 'agent2.md'), '# Agent 2');

        await syncAgentsAndSkills(tempSourceRoot, tempTargetRoot);

        // Verify agents were copied
        const agent1Content = await mockFsPromises.readFile(join(tempTargetRoot, 'agents', 'agent1.md'), 'utf8');
        const agent2Content = await mockFsPromises.readFile(join(tempTargetRoot, 'agents', 'agent2.md'), 'utf8');
    ...
    test('should copy skills from source to target', async () => {
        ...
        await mockFsPromises.mkdir(join(tempSourceRoot, 'agents'), { recursive: true });
        await mockFsPromises.mkdir(join(tempSourceRoot, 'skills'), { recursive: true });
        await mockFsPromises.writeFile(join(tempSourceRoot, 'skills', 'skill1.md'), '# Skill 1');
        await mockFsPromises.writeFile(join(tempSourceRoot, 'skills', 'skill2.md'), '# Skill 2');

        await syncAgentsAndSkills(tempSourceRoot, tempTargetRoot);

        // Verify skills were copied
        const skill1Content = await mockFsPromises.readFile(join(tempTargetRoot, 'skills', 'skill1.md'), 'utf8');
        const skill2Content = await mockFsPromises.readFile(join(tempTargetRoot, 'skills', 'skill2.md'), 'utf8');
    ...
    test('should clear existing target directory contents before copying', async () => {
        ...
        await mockFsPromises.mkdir(join(tempSourceRoot, 'agents'), { recursive: true });
        await mockFsPromises.mkdir(join(tempSourceRoot, 'skills'), { recursive: true });
        await mockFsPromises.mkdir(join(tempTargetRoot, 'agents'), { recursive: true });
        await mockFsPromises.mkdir(join(tempTargetRoot, 'skills'), { recursive: true });

        // Add stale files in target
        await mockFsPromises.writeFile(join(tempTargetRoot, 'agents', 'stale-agent.md'), '# Stale Agent');
        await mockFsPromises.writeFile(join(tempTargetRoot, 'skills', 'stale-skill.md'), '# Stale Skill');

        // Add new files in source
        await mockFsPromises.writeFile(join(tempSourceRoot, 'agents', 'new-agent.md'), '# New Agent');
        await mockFsPromises.writeFile(join(tempSourceRoot, 'skills', 'new-skill.md'), '# New Skill');
    ...
    test('should handle empty source agents directory', async () => {
        ...
        await mockFsPromises.mkdir(join(tempTargetRoot, 'agents'), { recursive: true });
        await mockFsPromises.writeFile(join(tempTargetRoot, 'agents', 'old-agent.md'), '# Old Agent');
    ...
    test('should handle empty source skills directory', async () => {
        ...
        // Add stale content in target
        await mockFsPromises.mkdir(join(tempTargetRoot, 'skills'), { recursive: true });
        await mockFsPromises.writeFile(join(tempTargetRoot, 'skills', 'old-skill.md'), '# Old Skill');
    ...
    test('should copy nested skill directories correctly', async () => {
        ...
        await mockFsPromises.mkdir(join(tempSourceRoot, 'agents'), { recursive: true });
        await mockFsPromises.mkdir(join(tempSourceRoot, 'skills', 'memory-reflection'), { recursive: true });
        await mockFsPromises.writeFile(join(tempSourceRoot, 'skills', 'memory-reflection', 'SKILL.md'), '# Memory Reflection Skill');
        await mockFsPromises.writeFile(join(tempSourceRoot, 'skills', 'memory-reflection', 'config.json'), '{"enabled": true}');

        await syncAgentsAndSkills(tempSourceRoot, tempTargetRoot);

        // Verify nested structure was copied
        const skillContent = await mockFsPromises.readFile(join(tempTargetRoot, 'skills', 'memory-reflection', 'SKILL.md'), 'utf8');
        const configContent = await mockFsPromises.readFile(join(tempTargetRoot, 'skills', 'memory-reflection', 'config.json'), 'utf8');
    ...
    test('should use COPYFILE_FICLONE flag when copying', async () => {
        ...
        await mockFsPromises.mkdir(join(tempSourceRoot, 'agents'), { recursive: true });
        await mockFsPromises.mkdir(join(tempSourceRoot, 'skills'), { recursive: true });
        await mockFsPromises.writeFile(join(tempSourceRoot, 'agents', 'agent.md'), '# Agent');

        await syncAgentsAndSkills(tempSourceRoot, tempTargetRoot);

        // Verify the copy worked (flag was used correctly)
        const agentContent = await mockFsPromises.readFile(join(tempTargetRoot, 'agents', 'agent.md'), 'utf8');
