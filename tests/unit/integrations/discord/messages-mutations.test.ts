    ...
    describe('splitMessage', () => {
        ...
        describe('mutation coverage - regex whitespace+', () => {
            ...
            test('should correctly split when forcing word-level processing', () => {
                ...
                const words = Array.from({ length: 10 }, (_, i) => `word${i}`).join('     '); // 5 spaces between
        ...
        describe('mutation coverage - regex n{2,} vs n', () => {
            ...
            test('should differentiate single vs double newline when forcing split', () => {
                ...
                const para1 = `${'x'.repeat(30)}\n${'y'.repeat(30)}`; // Single newline inside: 61 chars
                const para2 = 'z'.repeat(30);
                ...
                const allText = result.join('');
                expect(allText).toContain('x'.repeat(30));
                expect(allText).toContain('y'.repeat(30));
                expect(allText).toContain('z'.repeat(30));
            });
        });
            ...
            describe('word-level flush', () => {
                test.each([
                    ...
                    },
                    {
                        ...
                        maxLength:      50,
                        expectedFirst:  'x'.repeat(50),
                        expectedLength: 2,
                        noEmpty:        true
                    },
                    {
                        desc:           'no empty between consecutive long words',
                        message:        `${'a'.repeat(60)} ${'b'.repeat(60)}`,
                        maxLength:      50,
                        expectedLength: 4,
                        noEmpty:        true,
                        expectedChunks: ['a'.repeat(50), 'a'.repeat(10), 'b'.repeat(50), 'b'.repeat(10)] as string[]
                    },
                    {
                        desc:           'reset after flush (no Stryker mutation)',
                        message:        `aa ${'x'.repeat(60)} bb`,
                        maxLength:      50,
                        expectedLength: 4,
                        noStryker:      true
                    }
        ...
        describe('mutation coverage - regex patterns', () => {
            ...
            test('should extract sentences with various punctuation patterns', () => {
                ...
                const sent1 = `${'a'.repeat(40)}.`;
                const sent2 = `${'b'.repeat(40)}!`;
                const sent3 = `${'c'.repeat(40)}?`;
