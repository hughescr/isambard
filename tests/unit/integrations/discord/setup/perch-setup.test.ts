...
describe('setupPerchDriverAndScheduler', () => {
    ...
    it('creates a driver with no stateManager/runner dependency and a scheduler with no isPerchTurnRunning', () => {
        ...
        const conductor = { submit: mock(async () => makeTurnResult()), interruptCurrent: mock(), status: mock(() => ({ role: 'perch' as const, sessionId: undefined, lifecycle: 'open' as const, queueLength: 0, turn: null })), deliver: mock(async () => ({ outcome: 'committed' as const, disposition: 'sent' as const })) };
    ...
    it('wires the scheduler\'s onPerchTrigger to call driver.runSlot', () => {
        ...
        setupPerchDriverAndScheduler({
            conductor:   { submit: mock(async () => makeTurnResult()), interruptCurrent: mock(), status: mock(() => ({ role: 'perch' as const, sessionId: undefined, lifecycle: 'open' as const, queueLength: 0, turn: null })), deliver: mock(async () => ({ outcome: 'committed' as const, disposition: 'sent' as const })) },
    ...
    describe('perch-turn result delivery (P12 folded gap)', () => {
        ...
        it('delivers a settled \'perch\'-kind turn\'s response to the well-known perch-time channel via conductor.deliver', async () => {
            ...
            setupPerchDriverAndScheduler({
                conductor:   { submit: innerSubmit, interruptCurrent: mock(), status: mock(() => ({ role: 'perch' as const, sessionId: undefined, lifecycle: 'open' as const, queueLength: 0, turn: null })), deliver: innerDeliver },
                perchConfig: PERCH_CONFIG,
                clock:       { now: () => 0, setTimer: mock(), clearTimer: mock() },
                ...deliveryDeps(),
            });
        ...
        it('forces a perch reply to perch-time even if its envelope carries an origin channel', async () => {
            ...
            setupPerchDriverAndScheduler({
                conductor:   { submit: mock(async () => makeTurnResult({ envelopeId: 'env-perch-origin', response: 'Perch reply' })), interruptCurrent: mock(), status: mock(() => ({ role: 'perch' as const, sessionId: undefined, lifecycle: 'open' as const, queueLength: 0, turn: null })), deliver: innerDeliver },
                perchConfig: PERCH_CONFIG,
                clock:       { now: () => 0, setTimer: mock(), clearTimer: mock() },
                ...deliveryDeps(),
            });
        ...
        it('never delivers a \'discord\'-kind turn (handlers.ts already delivers that one itself)', async () => {
            ...
            setupPerchDriverAndScheduler({
                conductor:   { submit: mock(async () => makeTurnResult({ envelopeId: 'env-discord-1', response: 'hi' })), interruptCurrent: mock(), status: mock(() => ({ role: 'perch' as const, sessionId: undefined, lifecycle: 'open' as const, queueLength: 0, turn: null })), deliver: innerDeliver },
                perchConfig: PERCH_CONFIG,
                clock:       { now: () => 0, setTimer: mock(), clearTimer: mock() },
                ...deliveryDeps(),
            });
        ...
        it('does not deliver when the turn produced no response (null)', async () => {
            ...
            setupPerchDriverAndScheduler({
                conductor:   { submit: mock(async () => makeInterruptedTurnResult('env-perch-2')), interruptCurrent: mock(), status: mock(() => ({ role: 'perch' as const, sessionId: undefined, lifecycle: 'open' as const, queueLength: 0, turn: null })), deliver: innerDeliver },
                perchConfig: PERCH_CONFIG,
                clock:       { now: () => 0, setTimer: mock(), clearTimer: mock() },
                ...deliveryDeps(),
            });
        ...
        it('a delivery failure is logged but does not reject the wrapped submit() call', async () => {
            ...
            setupPerchDriverAndScheduler({
                conductor:   { submit: mock(async () => makeTurnResult({ envelopeId: 'env-perch-3', response: 'text' })), interruptCurrent: mock(), status: mock(() => ({ role: 'perch' as const, sessionId: undefined, lifecycle: 'open' as const, queueLength: 0, turn: null })), deliver: innerDeliver },
                perchConfig: PERCH_CONFIG,
                clock:       { now: () => 0, setTimer: mock(), clearTimer: mock() },
                ...deliveryDeps(),
            });
        ...
        it('the not-sent sentinel (sent:false, queued:false) is suppressed — no error log, no rejection of the wrapped submit() call', async () => {
            ...
            setupPerchDriverAndScheduler({
                conductor:   { submit: mock(async () => makeTurnResult({ envelopeId: 'env-perch-4', response: 'text' })), interruptCurrent: mock(), status: mock(() => ({ role: 'perch' as const, sessionId: undefined, lifecycle: 'open' as const, queueLength: 0, turn: null })), deliver: innerDeliver },
                perchConfig: PERCH_CONFIG,
                clock:       { now: () => 0, setTimer: mock(), clearTimer: mock() },
                ...deliveryDeps(),
            });
        ...
        it('a queued perch response completes delivery and returns the empty-channel receipt', async () => {
            ...
            setupPerchDriverAndScheduler({
                conductor:   { submit: mock(async () => makeTurnResult({ response: 'queued' })), interruptCurrent: mock(), status: mock(() => ({ role: 'perch' as const, sessionId: undefined, lifecycle: 'open' as const, queueLength: 0, turn: null })), deliver: innerDeliver },
                perchConfig: PERCH_CONFIG,
                clock:       { now: () => 0, setTimer: mock(), clearTimer: mock() },
                ...deliveryDeps(),
            });
